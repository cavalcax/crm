...
// Fetch Event Info
$stmt = $pdo->prepare("SELECT * FROM " . TABLE_NAME . "schedule WHERE id = ?");
$stmt->execute([$event_id]);
$event = $stmt->fetch();

if (!$event || (!$isAdmin && $event['user_id'] != $user_id)) {
    echo "Compromisso não encontrado ou acesso negado.";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title && $date && $time) {
        $stmt = $pdo->prepare("
            ...
            SET client_id = ?, title = ?, type = ?, start_time = ?, observation = ?, address = ?, city = ?, uf = ?, latitude = ?, longitude = ?, banner_image = ?, auction_lots_link = ?, auction_live_link = ? 
            WHERE id = ?
        ");
        $stmt->execute([$client_id, $title, $type, $start_time, $obs, $address, $city, $uf, $latitude, $longitude, $banner_image, $auction_lots_link, $auction_live_link, $event_id]);
...
$eventTime = $startDate->format('H:i');
?>
<!DOCTYPE html>
<html lang="pt-BR">
            ...
            <main class="flex-1 overflow-x-hidden overflow-y-auto bg-brand-50 p-6">
                ...
                <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-2xl p-6 sm:p-8 mb-10">
                    <div class="flex justify-between items-center mb-6 pb-4 border-b border-gray-200">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-800">Editar Compromisso</h2>
                        </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="time" name="time"
                                            class="shadow-sm appearance-none border border-gray-300 rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-brand-500"
                                            value="<?php echo $eventTime; ?>" required>
                                    </div>
                                    <div>
                                        ...
                                        <select name="type" id="eventTypeSelect"
                                            ...
                                            <option value="meeting" <?php echo $event['type'] === 'meeting' ? 'selected' : ''; ?>>Reunião</option>
                                            <option value="visit" <?php echo $event['type'] === 'visit' ? 'selected' : ''; ?>>Visita</option>
                                            <option value="auction" <?php echo $event['type'] === 'auction' ? 'selected' : ''; ?>>Leilão</option>
                                            <option value="other" <?php echo $event['type'] === 'other' ? 'selected' : ''; ?>>Outro</option>
                                        </select>
                                    </div>
                                </div>

                                <!-- Auction Specific Fields (Conditional) -->
                                <div id="auctionFieldsSection" class="<?php echo $event['type'] === 'auction' ? '' : 'hidden'; ?> p-4 bg-amber-50/70 rounded-xl border border-amber-200 space-y-4 transition">
                                    ...
                                    <div>
                                        ...
                                        <?php if (!empty($event['banner_image'])): ?>
                                            <div class="mb-2 flex items-center gap-3">
                                                <img src="../<?php echo htmlspecialchars($event['banner_image']); ?>" alt="Banner do Leilão" class="h-16 w-28 object-cover rounded-lg border shadow-sm">
                                                <span class="text-xs text-gray-500">Banner atual carregado. Selecione novo arquivo para alterar.</span>
                                            </div>
                                        <?php endif; ?>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input type="url" name="auction_lots_link"
                                                value="<?php echo htmlspecialchars($event['auction_lots_link'] ?? ''); ?>"
                                        ...
                                        <div>
                                            ...
                                            <input type="url" name="auction_live_link"
                                                value="<?php echo htmlspecialchars($event['auction_live_link'] ?? ''); ?>"
                                ...
                                <div>
                                    ...
                                    <select name="uf" id="ufInput"
                                        ...
                                        <?php foreach ($states as $code => $stateName): ?>
                                            <option value="<?php echo $code; ?>" <?php echo ($event['uf'] ?? '') === $code ? 'selected' : ''; ?>>
                            ...
                            <div class="mb-4">
                                ...
                                <p class="text-xs text-gray-500 mt-2" id="geo-feedback">
                                    <?php if (!empty($event['latitude']) && !empty($event['longitude'])): ?>
                                        Lat: <?php echo number_format($event['latitude'], 6); ?>, Long: <?php echo number_format($event['longitude'], 6); ?>
                                    <?php else: ?>
                                        Lat: -, Long: -
                                    <?php endif; ?>
                                </p>
                        ...
                        <div class="pt-4">
                            ...
                            <div>
                                <textarea name="observation" rows="3"
                                    ...
                                    class="shadow-sm appearance-none border border-gray-300 rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-brand-500"><?php echo htmlspecialchars($event['observation'] ?? ''); ?></textarea>
                            </div>
                        </div>
    ...
    <script>
        ...
        function initMap() {
            ...
            marker = new google.maps.Marker({
                position: (initialLat && initialLng) ? defaultPos : null,
                map: map,
                visible: (initialLat && initialLng) ? true : false,
                draggable: true
            });
