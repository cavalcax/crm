...
// Fetch Event Info
$stmt = $pdo->prepare("SELECT * FROM " . TABLE_NAME . "schedule WHERE id = ? AND user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title && $date && $time) {
        $stmt = $pdo->prepare("
            ...
            SET client_id = ?, title = ?, type = ?, start_time = ?, observation = ?, address = ?, city = ?, uf = ?, latitude = ?, longitude = ? 
            WHERE id = ? AND user_id = ?
        ");
        $stmt->execute([$client_id, $title, $type, $start_time, $obs, $address, $city, $uf, $latitude, $longitude, $event_id, $user_id]);
